class GroceryData {
  static List<Map<String, dynamic>> groceryProducts = [
    {
      'id': 1,
      'name': 'Apples',
      'description': 'Fresh and crisp apples',
      'price': 2.99,
      'imageUrl':
          'https://www.shutterstock.com/image-photo/red-apple-isolated-on-white-600nw-1727544364.jpg',
    },
    {
      'id': 2,
      'name': 'Milk',
      'description': 'Whole milk, 1 gallon',
      'price': 1.99,
      'imageUrl':
          'https://static.wixstatic.com/media/cbc1ea_599238dbe91647918df33fc6c049268c~mv2.png/v1/fill/w_640,h_374,al_c,q_85,usm_0.66_1.00_0.01,enc_auto/cbc1ea_599238dbe91647918df33fc6c049268c~mv2.png',
    },
    {
      'id': 3,
      'name': 'Bread',
      'description': 'Whole grain bread',
      'price': 2.49,
      'imageUrl':
          'https://www.foodandwine.com/thmb/Z2AE53BGYP2U-kXhtbVwXZQX8sc=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Perfect-Sandwich-Bread-FT-RECIPE0723-dace53e15a304942acbc880b0ae34f5a.jpg',
    },
    {
      'id': 4,
      'name': 'Chicken',
      'description': 'Boneless chicken breast',
      'price': 5.99,
      'imageUrl':
          'https://www.bigsams.in/wp-content/uploads/2022/04/CHICKEN-LEG-BONELESS-500G-2.jpg',
    },
    {
      'id': 5,
      'name': 'Eggs',
      'description': 'Dozen large eggs',
      'price': 1.79,
      'imageUrl':
          'https://www.licious.in/blog/wp-content/uploads/2022/01/eggs-1-1024x1024.jpg',
    },
  ];
}
